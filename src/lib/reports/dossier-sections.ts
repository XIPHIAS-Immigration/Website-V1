...
export function buildDossierPages(dossier: Dossier, opts: DossierOpts): string[] {
  ...
  let ptr = 1;
  const img = (): string | undefined => (imgs.length ? imgs[ptr++ % imgs.length] : undefined);
  ...
  if (want("divider"))
    pages.push(imageDividerPage({ imageDataUri: img(), eyebrow: `${vLabel} programme`, title, desc: dossier.tagline }));

  // 1 — Snapshot (split: facts + benefits | tall image)
  if (want("snapshot")) {
  const factCards = [
    card({ k: "Destination", v: country || "—" }),
  ...
  const snapshot =
    ...
    grid(2, factCards) +
    (benefits.length ? `<div class="spacer-16"></div><h3 class="h-sub">Key benefits</h3>` + ticks(benefits) : "") +
    (tagPills ? `<div class="spacer-8"></div><div>${tagPills}</div>` : "");
  addSplit("Snapshot", snapshot, title);
  }
  ...
  if (want("eligibility") && (requirements.length || langTests.length || disqualifiers.length)) {
    let body = sectionHeader({ eyebrow: "Eligibility", title: "How you qualify", desc: "The criteria this route is assessed against — confirmed at advisor review." });
  ...
  if (want("scoring") && (pointsRows.length || occBlocks)) {
    ...
    if (pointsRows.length) body += table({ head: ["Category", "Max", "Notes"], rows: pointsRows.map((r) => [esc(r.category), esc(String(r.max ?? "—")), esc(r.notes || "")]) });
  ...
  if (want("costs") && (prices.length || govFees.length || pof.length || (dossier.minInvestment ?? 0) > 0)) {
    let body = sectionHeader({ eyebrow: "Commercial view", title: "Indicative costs & fees", desc: "Programme, government and professional cost lines. Final quotation follows advisor verification." });
    body +=
      bigStats(
        [
          (dossier.minInvestment ?? 0) > 0 ? { k: "Investment from", v: money(dossier.minInvestment, dossier.currency) } : null,
          { k: "Timeline", v: dossier.timelineLabel || (dossier.timelineMonths ? `${dossier.timelineMonths} mo` : "—") },
    ...
    addAuto("Costs", body, "Costs & fees", prices.length + govFees.length + pof.length >= 8);
  }
  ...
  if (want("documents") && docGroups.length) {
    ...
    chunk(groupsHtml, 4).forEach((gc, i) => {
      ...
      addFull("Documents", head + gc.join(`<div class="spacer-8"></div>`));
    });
  }
  ...
  if (want("process") && stepItems.length)
    addSplit(
      "Process",
      sectionHeader({ eyebrow: "Execution sequence", title: "How the application progresses", desc: "Indicative only — government processing and document readiness can change the plan." }) + steps(stepItems),
      "Process timeline",
    );
